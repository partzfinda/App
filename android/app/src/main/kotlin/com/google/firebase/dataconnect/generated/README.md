...
is the query named "GetPartRequestsByMechanic",
which can be accessed via the [ExampleConnector.getPartRequestsByMechanic] property.


An example of the property for a mutation
is the mutation named "CreateShopProfile",
which can be accessed via the [ExampleConnector.createShopProfile] property.
...
For example, the "ListShopProfiles" query has no variables
and can be executed via the
[ExampleConnector.listShopProfiles]
property as follows:

```kotlin
val connector = ExampleConnector.instance
val queryResult = connector.listShopProfiles.execute()
println("ListShopProfiles query returned: ${queryResult.data}")
```
...
For example, the "GetPartRequestsByMechanic" query has 1 required variable ("mechanicId")
and can be executed via the [ExampleConnector.getPartRequestsByMechanic]
property as follows:

```kotlin
val connector = ExampleConnector.instance
val queryResult = connector.getPartRequestsByMechanic.execute(mechanicId=java.util.UUID.randomUUID())
println("GetPartRequestsByMechanic query returned: ${queryResult.data}")
```
...
For example, the "InsertPartRequest" mutation has 5 required variables ("mechanicId", "partDescription", "vehicleMake", "vehicleModel", and "vehicleYear")
and can be executed via the [ExampleConnector.insertPartRequest]
property as follows:

```kotlin
val connector = ExampleConnector.instance
val mutationResult = connector.insertPartRequest.execute(mechanicId=java.util.UUID.randomUUID(), partDescription="quux", vehicleMake="corge", vehicleModel="waldo", vehicleYear=1039)
println("InsertPartRequest mutation returned: ${mutationResult.data}")
```
...
For example, the "CreateShopProfile" mutation has 2 optional variables ("deliveryOptions" and "inventorySpecialties")
and can be executed via the [ExampleConnector.createShopProfile]
property as follows:

```kotlin
val connector = ExampleConnector.instance
val mutationResult = connector.createShopProfile.execute(userId=java.util.UUID.randomUUID(), primaryContactPerson="fred", shopName="baz") {
  deliveryOptions = listOf(...)
  inventorySpecialties = listOf(...)
}
println("CreateShopProfile mutation returned: ${mutationResult.data}")
```
